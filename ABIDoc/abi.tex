...
Each entry in the \ccode{__objc_class_refs} section is a symbol (in a COMDAT of the same name) called \ccode{._OBJC_REF_CLASS_\{class name\}}, which is initialised to point to a variable called \ccode{._OBJC_CLASS_\{class name\}}, which is the symbol for the class.
This and the class structure are the \textit{only} place where the \ccode{._OBJC_CLASS_\{class name\}} symbols may be referenced.

All other accesses to the class (i.e. from message sends to classes or to \objc{super}) must be via a load of the \ccode {._OBJC_REF_CLASS_\{class name\}} variable.
...
Categories provide a mechanism for adding methods and properties to existing classes.
For each category, the compiler generates a structure as shown in \Fref{lst:category} in the \ccode{__objc_cats} section.

\inccode{category.h}{category}{objc_category}{The category structure.}

The \ccode{name} field specifies the name of this category.
This is used solely for debugging.
The \ccode{class_name} field specifies the name of the class.
This is used instead of a direct reference to the class structure to allow categories to be loaded independently of the class to which they are applied.

The remaining fields contain the same structures as classes and are described in \Fref{chap:class}.
Each of these structures contains a \ccode{next} field that allows the category versions to be added to the front of a linked list.
All of the structures attached to classes are allowed in categories, with the exception of instance variables (which would change the class size).

\sloppy
Categories define extra state for a class \textit{pair}.
Objective-C classes follow the Smalltalk model, where classes describe objects and metaclasses describe objects.
There is no analogous metacategory structure: the \ccode{class_methods} and \ccode{class_properties} fields in a category apply to the metaclass, whereas the \ccode{instance_methods} and \ccode{properties} fields refer to the class.

Note that there is no \ccode{class_protocols} field.
Objective-C protocols describe both class and instance methods (somewhat breaking the metaclass model) and so are applied to the class and not the metaclass, even if they describe only class methods.

\chapter{Protocols}
\label{chap:protocols}

Objective-C protocols are represented by a structure with the name \ccode{._OBJC_PROTOCOL_{Protocol name}} as shown in \Fref{lst:protocol}, in the \ccode{__objc_protocols} section.
The compiler is responsible for also emitting a symbol with the name \ccode{._OBJC_REF_PROTOCOL_{Protocol name}} in the \ccode{__objc_protocol_refs} section, initialised to point to the first symbol.
All references from \objc{@protocol} directives must load from the second symbol.
Both symbols are in COMDATs and have hidden visibility.
It is the responsibility of the linker to ensure that only one instance of a protocol exists in a binary and the responsibility of the runtime to ensure that all of the reference variables point to the canonical copy of the protocol structure.

The protocol structure contains an \ccode{isa} pointer, which is initialised by the compiler to the version of the protocol.
The version described in this document is 4.
Version 3 was used by the previous GNUstep ABI, version 2 by the GCC runtime.

The \ccode{name} field contains the name of the protocol, as a null-terminated C string.
The \ccode{protocol_list} field describes the protocols that this protocol conforms to.
This structure is identical to the version used by classes, described in \Fref{sec:protocollist}.

Protocols can define properties that are either optional or required and apply to either the class or the instance.
These are defined by the last four fields, which are identical to their counterparts in the class structure, descried in \Fref{sec:propertylis}
...
Instead, they use an array described in \Fref{lst:protocolmethodlist} of the method description structures described in \Fref{lst:protocolmethod}.
